...
impl SolverCtx {
    ...
    pub fn test_concrete_with_static_widths(
        ...
        for (i, a) in assumptions.iter().enumerate() {
            self.smt
                .assert(self.smt.named(format!("conc{i}"), *a))
                .unwrap();
        }
        for (i, e) in self.additional_assertions.iter().enumerate() {
            self.smt
                .assert(self.smt.named(format!("conc_assert{i}"), *e))
                .unwrap();
        }
        self.smt.push().unwrap();
        let eq = self
            .smt
            .eq(rhs, self.smt.atom(concrete.output.literal.clone()));

        self.smt
            .assert(self.smt.named("conceq".to_string(), eq))
            .unwrap();

        for (i, a) in rulectx.rule_sem.rhs_assertions.iter().enumerate() {
            let p = self.vir_expr_to_sexp(a.clone());
            self.smt
                .assert(self.smt.named(format!("rhs_assert{i}"), p))
                .unwrap();
        }

        if !matches!(self.smt.check(), Ok(Response::Sat)) {
            // Bad! This is a bug!
            // Pop the output assertion
            self.smt.pop().unwrap();
            // Try again
            assert!(matches!(self.smt.check(), Ok(Response::Sat)));
            // Get the value for what output is to panic with a useful message
            let val = self.smt.get_value(vec![rhs]).unwrap()[0].1;
            self.display_model(rulectx.termenv, rulectx.typeenv, rulectx.rule, lhs, rhs);
            panic!(
                "Expected {}, got {}",
                concrete.output.literal,
                self.display_hex_to_bin(val)
            );
        } else {
            log::debug!(
        ...
        self.smt
            ...
            .unwrap();
        if !matches!(self.smt.check(), Ok(Response::Unsat)) {
            // Get the value for what output is to panic with a useful message
            let val = self.smt.get_value(vec![rhs]).unwrap()[0].1;
            self.display_model(rulectx.termenv, rulectx.typeenv, rulectx.rule, lhs, rhs);
...
pub fn run_solver_with_static_widths(
    ...
    if let Some(concrete) = concrete {
        ctx.test_concrete_with_static_widths(
            rulectx,
            concrete,
            lhs,
            rhs,
